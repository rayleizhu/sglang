...
# NOTE: the original subclasses transformers.cache_utils.Cache and calls
# super().__init__().  Newer transformers (>=4.53) changed Cache.__init__ to
# require a `layers`/`layer_class_to_replicate` argument, which breaks that
# call.  KCompressionCache is just a per-layer container and uses none of the
# base class's machinery, so we drop the inheritance for version robustness.
