...
from ..flash_compat import sdpa_varlen_causal
from ..kernels.varlen.block_sparse_flash_decode_varlen_mask_leftpad import block_sparse_flash_decode_leftpad_gqa_mask
...
def sparse_flash_attention_forward(
    ...
    **kwargs,
    ...
    if query_length > 1:
        ...
        # SDPA-based causal prefill (equivalent to flash_attn_varlen_func; the
        # block sparsity only applies at decode, so prefill is always dense).
        attn_output = sdpa_varlen_causal(
            query_states,
            key_states,
            value_states,
